...
function Footer() { return <footer><div className="container footergrid"><div><Brand/><p>Uganda&apos;s specialist centre for musculoskeletal health, workplace wellness and physical performance. Founded by licensed physiotherapists. Built for Uganda.</p></div><div><h4>Quick links</h4>{nav.slice(1,5).map(([l,h]) => <Link key={h} href={h}>{l}</Link>)}</div><div><h4>Our divisions</h4><Link href="/divisions">CoreWell Corporate</Link><Link href="/divisions">CoreWell Spine Clinic</Link><Link href="/divisions">CoreWell Performance</Link></div><div><h4>Contact</h4><p>People&apos;s Medical Hospital<br/>Gayaza, Kampala, Uganda</p><a href="tel:+256761393569">+256 761 393 569</a><a href="mailto:info@corewellmusculoskeletaluganda.com">info@corewellmusculoskeletaluganda.com</a></div></div><div className="container footnote"><span>© 2026 CoreWell Uganda Limited · Registered in Uganda · URSB</span><span><a href="#">Privacy Policy</a><a href="#">Terms &amp; Conditions</a></span></div></footer>; }
...
function TeamCards({ full=false }: {full?:boolean}) { const team = [
  { image: "/images/team/julius.jpeg", name: "Julius Kaweesa", role: "Co-Founder & Lead Clinician", phone: "+256 761 393 569", email: "julius@corewellmusculoskeletaluganda.com", bio: "Julius is the founding architect of CoreWell Uganda's clinical and corporate model. His experience in musculoskeletal practice revealed the growing burden of preventable pain in Uganda's workplaces and shaped the company's response. He developed CoreWell's 56-day progressive back pain programme and leads business development and clinical governance, with a focus on spinal rehabilitation, occupational health, corporate wellness, MSK ultrasound and back pain management." },
  { image: "/images/team/emma.jpeg", name: "Kajwiga Emmanuel", role: "Co-Founder & Clinical Partner", phone: "+256 784 106 753", email: "emma@corewellmusculoskeletaluganda.com", bio: "Kajwiga is CoreWell Uganda's clinical partner and co-lead. His work spans musculoskeletal, sports and complex rehabilitation, including experience with Uganda's golf community at high levels. That clinical insight informs CoreWell Performance and its golf physiotherapy offering, while he also co-leads corporate programme delivery." },
]; return <div className={full ? "teamgrid teamgrid-full" : "teamgrid"}>{team.map(({image,name,role,phone,email,bio}) => <article className="teamcard" key={name}><div className="portrait"><img src={image} alt={`${name}, Licensed Physiotherapist and CoreWell Uganda Co-Founder`}/></div><div className="teamcopy"><span className="license">Licensed Physiotherapist</span><h3>{name}</h3><p>{role}</p>{full ? <><p>{bio}</p><div className="teamcontact"><a href={`tel:${phone.replace(/\s/g,"")}`}>{phone}</a><a href={`mailto:${email}`}>{email}</a></div></> : <Link className="textlink" href="/team">View profile <span>→</span></Link>}</div></article>)}</div>; }
...
function HomePage(){
  return <>
    ...
    <section className="section"><div className="container"><SectionHead eyebrow="Our founders" title="Clinicians first. Business builders second."/><TeamCards/></div></section>
    <section className="quote-strip"><div className="container quotegrid"><blockquote>“We did not create this problem. We found it — in our clinics, in our data, in the stories our patients told us about their workdays. CoreWell Uganda was built to solve it.”<cite>CoreWell Uganda founders</cite></blockquote><div className="stat"><b>Licensed</b><span>physiotherapists only</span></div><div className="stat"><b>3</b><span>specialist divisions</span></div><div className="stat"><b>1</b><span>evidence-based standard</span></div></div></section>
...
function ContactSection(){ return <section className="section contactsection"><div className="container contactgrid"><div className="contactintro"><SectionHead eyebrow="Contact us" title="Ready to protect your workforce, your patients, your game?" copy="Whether you are an HR manager, a patient or an athlete, CoreWell Uganda has a service designed for you. Reach out and let us start the conversation."/><div className="contactlist"><p><b>Clinic</b>People&apos;s Medical Hospital<br/>Gayaza, Kampala, Uganda<br/>Monday–Saturday, 8am–6pm</p><p><b>Phone</b><a href="tel:+256761393569">+256 761 393 569</a><a href="tel:+256784106753">+256 784 106 753</a></p><p><b>Email</b><a href="mailto:info@corewellmusculoskeletaluganda.com">info@corewellmusculoskeletaluganda.com</a></p></div></div><ContactForm/><iframe className="map" title="Map showing People's Medical Hospital in Gayaza" loading="lazy" src="https://www.google.com/maps?q=People%27s%20Medical%20Hospital%20Gayaza%20Kampala%20Uganda&output=embed"/></div></section>; }
